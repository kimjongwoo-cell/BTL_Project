...
class RingBeamDataset(Dataset):
    def __init__(self, root_path, classes: list[str], target_size=(256, 256), normalize=True, crop=False, crop_coords=(0,0,0,0)):
        ...
        for class_name in self.class_names:
            image_dir = os.path.join(self.root_path, class_name)
            ...
            for fname in os.listdir(image_dir):
                if fname.lower().endswith('.tiff') or fname.lower().endswith('.tif'):
                    self.items.append((os.path.join(image_dir, fname), self.class_to_idx[class_name]))
