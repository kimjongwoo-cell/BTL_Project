...
def main():
    ...
    parser.add_argument('--target_img_size', type=int, default=1024, help='Target size (width and height) to resize images to after cropping and after polar transform.')
    ...
    target_size = (args.target_img_size, args.target_img_size)
    ...
    with torch.no_grad():
        for img_full, label, fname in tqdm(data_loader, desc="Extracting Features (TIFF-Color Patch)"):
            ...
            if args.crop:
                ...
                img_cropped = img_full[y0:y1, x0:x1]
                if img_cropped.size > 0:
                    img_full = cv2.resize(img_cropped, target_size, interpolation=cv2.INTER_LINEAR)
                else:
                    print(f"[WARN] Cropping resulted in an empty image for {fname}. Skipping.")
                    continue
            else:
                img_full = cv2.resize(img_full, target_size, interpolation=cv2.INTER_LINEAR)
            ...
            if args.use_polar:
                ...
                polar_coarse, rs_coarse= _polar_resample(
                ...
                work_img = cv2.resize(polar_img, target_size, interpolation=cv2.INTER_LINEAR)
            else:
                work_img = img_full
